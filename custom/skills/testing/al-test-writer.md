...
Emit one capped agent finding per generated test codeunit with the generated AL in `suggested-code`; curated testing knowledge informs the artifact but is cited only if the input or generated artifact concretely violates it, never merely because the artifact satisfies it. When a target lacks a behaviour spec, return `partial` with the missing target named in `outcome-reason`; do not emit an applicability-only info finding. Hold every finding to the precision bar in `skills/do.md`.
