...
- FlowFields and CalcFormulas: aggregations callers re-derive that a `CalcFormula` could hold and `CalcFields` inside a hot loop.
...
Set `domain` to `Table Design` on every finding. For a local mechanical change, `suggested-code` carries the literal replacement. When the caller requests complete artifacts and a behavior-preserving result is determinable, the skill MUST emit both a complete table replacement finding spanning the original table and a separate sibling-codeunit artifact finding whose `location.file` is the requested new path, `location.line` is 1, and `suggested-code` is the complete new codeunit. Both findings still describe the concrete refactoring concern; artifact generation does not suppress reporting. When complete artifacts are requested but cannot be determined safely, return `partial` and explain the missing design input. Do not emit findings for satisfied rules.
