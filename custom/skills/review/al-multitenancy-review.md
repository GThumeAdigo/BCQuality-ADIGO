...
Output conforms to the DO output contract. Every finding sets `domain: Multi-tenancy`. Findings without a knowledge file are agent findings (`references: []`, `agent:` id, severity capped at `minor` and confidence capped at `medium`); findings citing a `security` or `integration` file carry that file path as `id` and primary reference.
