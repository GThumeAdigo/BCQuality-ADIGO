...
Sub-skills MAY also emit `suggested-code` when their knowledge file unambiguously implies the replacement (the `.good.al.txt` and `.bad.al.txt` companion examples are useful here). The super-skill copies the field through unchanged.
